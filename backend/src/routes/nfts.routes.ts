...
import { createNftSchema, listNftsQuerySchema, nftIdParamSchema } from "../validators/nft.validators.js";

export const nftsRouter = Router();

nftsRouter.get("/", validate(listNftsQuerySchema, "query"), asyncHandler(list));
nftsRouter.get("/:id", validate(nftIdParamSchema, "params"), asyncHandler(getById));
